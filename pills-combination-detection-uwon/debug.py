...
def register_pills_image(image_path):
    ...
    for i, contour in enumerate(tmpl_contours):
        ...
        # cv2.imshow(pills_combination['name'] + " check " + str(i), tmp)
        # cv2.waitKey(0)
        # cv2.destroyAllWindows()
...
print(tmpl_pills['center'])
img = cv2.imread("images/tmpl_pills.jpg")
tmpl_image = resizeFixRate(img, RESIZE_WIDTH)
x=135; y=408; w=20; h=20         # roi 좌표
roi = tmpl_image[y:y+h, x:x+w]   # roi 지정

cv2.rectangle(roi, (0,0), (h-1, w-1), (0,255,0))  # roi 전체에 사각형 그리기
cv2.imshow("img", tmpl_image)
pil_image=Image.fromarray(roi)
pil_image.save("images/test1.jpg")
#################---------------2---------------#################
img2 = cv2.imread("images/tmpl_pills.jpg")
tmpl_image2 = resizeFixRate(img2, RESIZE_WIDTH)
fix_img = cv2.cvtColor(tmpl_image2, cv2.COLOR_BGR2RGB)
x=241; y=426; w=20; h=20         # roi 좌표
roi2 = fix_img[y:y+h, x:x+w]   # roi 지정

cv2.rectangle(roi2, (0,0), (h-1, w-1), (0,255,0))  # roi 전체에 사각형 그리기
cv2.imshow("img2", fix_img)
pil_image2=Image.fromarray(roi2)
pil_image2.show()
pil_image2.save("images/test2.jpg")
#################---------------3---------------#################

img3 = cv2.imread("images/tmpl_pills.jpg")
tmpl_image3 = resizeFixRate(img3, RESIZE_WIDTH)
x=348; y=417; w=20; h=20         # roi 좌표
roi3 = tmpl_image3[y:y+h, x:x+w]   # roi 지정

cv2.rectangle(roi3, (0,0), (h-1, w-1), (0,255,0))  # roi 전체에 사각형 그리기
cv2.imshow("img3", tmpl_image3)
pil_image3=Image.fromarray(roi3)
pil_image3.save("images/test3.jpg")
########################################################################
